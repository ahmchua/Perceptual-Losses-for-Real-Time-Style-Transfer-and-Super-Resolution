...
class SuperResolutionGrader(Grader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for img in data_loader():
            ...
            img_recs = [superresolve(img_low, factor, seed=seed) for seed in range(10)]
            # from PIL import Image
            # img_recs = [Image.fromarray((np.array(superresolve(img_low, seed=seed),dtype=np.uint8)+np.random.normal(scale=10.)).astype(np.uint8)) for seed in range(10)]
            # img_rec = img_recs[0]
